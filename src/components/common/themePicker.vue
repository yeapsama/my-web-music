<template>
  <div class="theme">
      <div class="theme-item" v-for="(item,index) in themeList" :key="index" @click="changeTheme(item,index)">
          <div class="bg" :style="{ background: item.themeColor }">
...
export default {
    ...
    created(){
        ...
        if(currentTheme != null){
            for(let t in currentTheme){
                if(t !='title'){
                    document.documentElement.style.setProperty(`--${t}`,currentTheme[t])
                }
            }
        }
    },
    methods:{
        changeTheme(item,index){
            ...
            for(let t in item){
                if(t != 'title'){
                    document.documentElement.style.setProperty(`--${t}`,item[t])
                }

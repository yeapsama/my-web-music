...
<script>
import { login, getQrKey, getLoginQr, checkLoginQr, getUserAccount, getAuthcode, checkAuthcode, register, checkPhoneNum } from "@/axiosReq/login/login";
import { getUserSonglist, getUserDetail } from "@/axiosReq/userdetail/userdetail";
import { getLikeSongList } from "@/axiosReq/playmusic/playmusic.js";
import { isPhone, throttle } from "@/utils/utils.js";
export default {
	name: "LoginPop",
	data() {
		//注册表单验证规则
		...
		return {
			PhoneNum: "", //登录手机号
			Password: "", //登录密码
			loginWay: 0, //登录方式		0手机号登录  1扫码登录   2注册
			qrurl: "", //二维码路径
			timer: "", //轮询二维码的定时器
            ...
            // 参考：https://www.jianshu.com/p/6a39dc155e88
            /*
            自定义校验 validator （validator是一个函数，有三个参数，rule(当前规则)，value(当前值)，callback（回调函数））
            校验触发方式trigger (change值改变时/blur失去焦点时)
            是否必填required（true/false)
            */
			rules: {
	...
	methods: {
				...
				login(this.PhoneNum, this.Password)
					.then(res => {
						if (res.data.code === 200) {
							...
							// 登录成功后的一些操作(关闭登录窗，将登录状态信息更新，获取用户歌单数据)
							this.loginSuccess(res);
		...
		loginSuccess(res) {
			...
			this.closeLogin();
			// 更新登录状态
			this.$store.dispatch("login/updateLogin", true);
			this.setItem("isLogin", true);
			//缓存用户信息 防止刷新消失
			this.setItem("userInfo", res.data.profile);
			// 提交vuex 保存用户信息
			this.$store.dispatch("login/saveUserInfo", res.data.profile);
			// 获取用户歌单数据
		...
		getUserSonglistBy(uid) {
			getUserSonglist(uid).then(res => {
				...
				this.$store.dispatch("login/saveUserSongList", res.data.playlist);
			});
			getLikeSongList(uid).then(res => {

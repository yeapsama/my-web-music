...
<script>
import { logout } from "@/axiosReq/login/login";
import { getUserDetail } from "@/axiosReq/userdetail/userdetail";
			...
			this.$confirm("您确定退出登录吗?", "提示", {
				...
				.then(() => {
					logout().then(res => {
						...
						// 更新登录状态
						this.$store.dispatch("login/updateLogin", false);
						this.removeItem("isLogin");
						// 清空用户信息
						this.$store.dispatch("login/saveUserInfo", null);
						this.removeItem("userInfo");
						// 清空歌单
						this.$store.dispatch("login/saveUserSongList", []);
						this.removeItem("userSongList");
						// 清空用户喜欢的音乐id列表
						this.$store.dispatch("login/saveLikeSongIds", []);

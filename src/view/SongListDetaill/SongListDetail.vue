		...
		<div class="bot-song-list" v-if="Object.keys(playlist).length !== 0">
			<el-tabs>
				...
				<el-tab-pane label="歌曲列表">
					<SongTable :songs="songList" @updateSonglist="updateSonglist" :currentPageNum="currentPageNum"/>
  	    			<Pagination :total="songs.length" :page-size="48" @handleCurrentChange="clickChangePage"/>
...
网络请求模块
 */
import { getSongListDetail, getEverySongDetail, getSongListSubscribers, subSongList } from "@/axiosReq/songlistdetail/songlistdetail.js";
import { getSongUrl } from "@/axiosReq/playmusic/playmusic.js";
// import { getUserSonglist } from "@/axiosReq/userdetail/userdetail";
import { mapGetters } from "vuex";
/* 
子组件 
*/
import Topdesc from "./ChildCompoments/TopDesc.vue";
import CommentPage from "@/components/content/comment/commentPage.vue";
import SongTable from "@/components/content/SongTable.vue";
import Subscribers from "./ChildCompoments/Subscriber.vue";
import Pagination from '@/components/common/Pagination'
export default {
	inject: ["reload"],
	name: "Songlistdetail",
	components: { Topdesc, CommentPage, SongTable, Subscribers , Pagination },
	data() {
		return {
			id: "", //存储路由传递过来歌单id
			playlist: {}, //歌单描述信息
    ...
    */
		...
		//分页器功能
		clickChangePage(page = 1){
			//48为分页器的偏移量
			let offset = (page - 1) * 48;
			if(this.songs.length <= 48){
				this.songList = this.songs;
			}else{
				if(page == 1){
					this.songList = this.songs.slice(1,49)
				}else if(offset+48 > this.songs.length){
					this.songList = this.songs.slice(offset)
				}else{
					this.songList = this.songs.slice(offset , offset + 48)
				}
			}
